...
// FARM DIMENSIONS: KEEP IN MIND THIS IS AN EAST-WEST BOT  SINCE IT WAS MADE FOR ELYSIUM
const startx = -917.5
const startz = -5379.5
...
// ITEM IDS
const tool = "minecraft:iron_axe" 
const crop = "minecraft:melon_slice" // the item you would like to farm, could also replace this with pumpkin
...
for (cz = startz; cz <= endz; cz += 5) {
    ...
    for (cx = startx; cx >= endx; cx--) {
        ...
        Player.getPlayer().lookAt(0, 68)
        pick(tool, 1, -1)
        KeyBind.key("key.mouse.left", true)
        Time.sleep(melonBreakTime)
        ...
        // (RIGHT) melons?
        Client.waitTick(10)
        Player.getPlayer().lookAt(180, 68)
        pick(tool, 1, -1)
        KeyBind.key("key.mouse.left", true)
        Time.sleep(melonBreakTime)
        KeyBind.key("key.mouse.left", false)
